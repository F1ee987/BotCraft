...
- `BotCraft.exe` — 桌面版（由 pywebview 打包，约 14 MB），双击即开、无需浏览器；已随仓库分发，需联网调用模型 API。
...
1. 获取文件：
   - 桌面版（推荐，最省事）：直接双击仓库内的 `BotCraft.exe`，无需浏览器、无需起服务，开箱即用；首次运行若被系统拦截，属正常 Windows 安全提示，允许运行即可。
